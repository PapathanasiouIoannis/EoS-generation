...
def _resolve_below(path: Path, parent: Path) -> Path:
    resolved = catalogue.confined(path, parent)
    if not resolved.exists():
        raise FileNotFoundError(resolved)
    return resolved


def _csv_label(value: str, field: str) -> str:
    """Reject spreadsheet formulas and control characters in text exports."""

    if not value or value[0] in "=+-@" or any(ord(char) < 32 for char in value):
        raise ValueError(f"unsafe {field} label: {value!r}")
    return value
...
def _source_catalogue(root: Path, run_root: Path, rank: int) -> pd.DataFrame:
    ...
    if not (run / "EOS_DATA").is_dir():
        return _source_experiment_catalogue(root, run, rank)
    ...
    table["_source_run"] = _csv_label(run.name, "source run")
    table["_run_root"] = str(run)
    table["_eos_data"] = str(eos_data)
    table["_manifest_sha256"] = catalogue.sha256(eos_data / "SHA256SUMS.txt")
    table["_source_mode"] = "legacy_eos_data"
    table["_experiment"] = ""
    return table


def _direct_experiment(run: Path) -> Path:
    """Resolve one sealed experiment below a notebook planning root."""

    if (run / "experiment.json").is_file():
        return run
    candidates = sorted(
        path.parent for path in run.glob("experiment_*/experiment.json")
        if path.is_file()
    )
    if len(candidates) != 1:
        raise ValueError(
            f"direct source must contain exactly one sealed experiment: {run}"
        )
    return candidates[0].resolve(strict=True)


def _direct_physical_key(
    *,
    amplitude: float,
    epsilon_match: float,
    center: float,
    ...
    if math.isclose(amplitude, 0.0):
        return "bsk24_analytic_baseline_v1"
    return "bsk24_windowed_curve:" + catalogue.digest(
        {
            "amplitude": amplitude,
            "epsilon_match_mev_fm3": epsilon_match,
            "epsilon0_mev_fm3": center,
            "sigma_mev_fm3": width,
            "delta_mev_fm3": ramp_width,
        }
    )


def _source_experiment_catalogue(
    root: Path, run: Path, rank: int
) -> pd.DataFrame:
    """Build the compact identity catalogue directly from sealed curve packets."""

    experiment = _direct_experiment(run)
    ...
    if not isinstance(children, list) or not children:
        raise ValueError(f"experiment has no child packets: {experiment}")
    records: list[dict[str, Any]] = []
    for child in children:
        packet = _resolve_below(experiment / str(child), experiment)
        if packet.parent != experiment:
            raise ValueError(f"child packet is not a direct experiment child: {child!r}")
        ...
        if not bool(config.get("curve_only_output", False)):
            raise ValueError(
                f"direct combined source is not curve-only: {packet}"
            )
        ledger = pd.read_csv(
        ...
        case_ids = tuple(dict.fromkeys(sequence["case_id"].astype(str)))
        for case_id in case_ids:
            if case_id == "direct":
                amplitude = 0.0
                status = "baseline"
                epsilon_match = center = width = ramp_width = float("nan")
            else:
                physical = (
                    ledger.get(
                        "physical_case_id", pd.Series("", index=ledger.index)
                    )
                    .astype(str)
                    .eq(case_id)
                )
                matches = ledger.loc[
                    ledger["case_id"].astype(str).eq(case_id) | physical
                ]
                matches = matches.loc[
                    matches["status"].astype(str).eq("accepted")
                ]
                if len(matches) != 1:
                    raise ValueError(
                        f"saved stellar case is not uniquely accepted: {packet}/{case_id}"
                    )
                match = matches.iloc[0]
                amplitude = float(match["amplitude"])
                if math.isclose(amplitude, 0.0):
                    # The owned A=0 logical case aliases the direct baseline.
                    continue
                status = "accepted"
                epsilon_match = float(match["epsilon_match_mev_fm3"])
                center = float(match["epsilon0_mev_fm3"])
                width = float(match["sigma_mev_fm3"])
                ramp_width = float(match["delta_mev_fm3"])
            physical_key = _direct_physical_key(
                amplitude=amplitude,
            ...
            records.append(
                {
                    "eos_id": f"{run.name}:{child}:{case_id}",
                    "catalogue_id": "bsk24_direct_curve_catalogue_v1",
                    "physical_model_key": physical_key,
                    "packet_path": packet.relative_to(root).as_posix(),
                    "geometry_id": str(child),
                    ...
                    "delta_mev_fm3": ramp_width,
                    "_source_rank": rank,
                    "_source_run": _csv_label(run.name, "source run"),
                    "_run_root": str(run),
                    "_eos_data": "",
                    "_manifest_sha256": catalogue.sha256(
                        experiment / "SHA256SUMS.txt"
                    ),
                    "_source_mode": "sealed_curve_experiment",
                    "_experiment": str(experiment),
                }
            )
    table = pd.DataFrame.from_records(records)
    if table.empty:
        raise ValueError(f"direct experiment has no saved stellar curves: {experiment}")
    return table
...
def build_name_mapping(
    ...
    for frame in frames:
        ...
        source = {
            ...
            "source_mode": str(first.get("_source_mode", "legacy_eos_data")),
            "catalogue_entry_count": int(len(frame)),
        }
        if source["source_mode"] == "sealed_curve_experiment":
            source.update(
                {
                    "experiment_path": _portable(
                        Path(first["_experiment"]), root
                    ),
                    "experiment_manifest_sha256": str(
                        first["_manifest_sha256"]
                    ),
                }
            )
        else:
            source.update(
                {
                    "eos_data_path": _portable(
                        Path(first["_eos_data"]), root
                    ),
                    "eos_data_manifest_sha256": str(
                        first["_manifest_sha256"]
                    ),
                }
            )
...
def finalize_manifest(
    repository_root: str | Path, destination: str | Path
) -> dict[str, Any]:
    root = Path(repository_root).resolve(strict=True)
    runs = (root / "runs").resolve(strict=True)
    target = _resolve_below(Path(destination), runs)
    if target == runs or target == catalogue.confined(runs / "eos_catalogue", runs):
        raise ValueError("manifest destination must be a derived dataset below runs/")
...
def build_combined_dataset(
    ...
    catalogue.require_disjoint(target, catalogue_root)
    source_paths = [_resolve_below(Path(path), runs) for path in source_runs]
    try:
        catalogue.require_disjoint(target, *source_paths)
    except ValueError as exc:
        raise ValueError("destination overlaps a source run") from exc
    ...
    try:
        ...
        (ml_data / "README.md").write_text(
            ...
            "These are derived saved-table outputs. Original sealed packets (and legacy "
            "EOS_DATA folders when supplied) remain authoritative and unchanged.\n",
            encoding="utf-8",
            newline="\n",
        )
...
def main() -> int:
    ...
    if args.finalize_manifest:
        if root is None:
            parser.error("--repository-root is required with --finalize-manifest")
        print(json.dumps(finalize_manifest(root, args.destination), sort_keys=True))
        return 0
    if root is None or not args.source_run:
        parser.error("--repository-root and at least two --source-run values are required")
    result = build_combined_dataset(
        root, args.source_run, args.destination
    )
