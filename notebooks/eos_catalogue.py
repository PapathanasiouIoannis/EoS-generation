...
MODEL_CONTRACTS = {
    "bsk24": {"prefix": "H", "baseline_family": "BSk24", "deformed_family": "deformed_BSk24"},
    "cfl": {"prefix": "C", "baseline_family": "CFL", "deformed_family": "deformed_CFL"},
}
PRIMARY_TABLES = (
...
def collect_sources(root: Path, experiment: Path) -> dict[str, Any]:
    ...
    for child, expected in zip(children, hashes, strict=True):
        ...
        if (metadata.get("packet_status") != "complete" or state.get("packet_status") != "complete"
                ...
                or metadata.get("baseline_validation_status") != "pass"
        ...
        for path in tables.values():
            with path.open(encoding="utf-8", newline="") as stream:
                ...
                if set(CONTEXT).intersection(reader.fieldnames):
...
def publish_directory(stage: Path, target: Path) -> None:
    ...
    for attempt in range(4):
        if target.exists() or target.is_symlink():
            raise FileExistsError(f"presentation destination already exists: {target}")
        try:
...
def build_eos_data(root: Path, experiment: Path, destination: Path) -> dict[str, Any]:
    ...
    if (target == runs or target == experiment or experiment in target.parents
            or target in experiment.parents or target == runs / "eos_catalogue"
            or runs / "eos_catalogue" in target.parents):
...
def main() -> int:
    ...
    print(json.dumps(build_eos_data(args.repository_root, args.experiment, args.destination), sort_keys=True))
    return 0
