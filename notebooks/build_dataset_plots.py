...
def build_dataset_plots(root: Path, experiment: Path, destination: Path, eos_data: Path) -> dict:
    ...
    eos_data = catalogue.confined(eos_data, runs)
    protected = (experiment, eos_data, runs / "eos_catalogue")
    ...
    curves, aliases = collect_curves(root, experiment, eos_data)
    ...
    try:
        ...
        catalogue.write_rows(stage / "accepted_case_index.csv", [c["alias"] for c in curves])
        ...
        provenance = {
            ...
            "eos_data_manifest_sha256": catalogue.sha256(eos_data / "SHA256SUMS.txt"),
        ...
        (stage / "README.md").write_text(
            "# Focused dataset plots\n\nFive figures combine accepted EoSs from this experiment only. "
            "Baseline identity controls are drawn once. Thermodynamic curves use retained profiles, "
            "not rejected raw proposals or the superluminal domain beyond the causal endpoint. "
            "The pressure-energy-density figure uses linear axes.\n\n"
            "Stellar curves use the final saved stage, in pressure order through the sampled peak. "
            "This prefix is not an independent stability certificate or a resolved maximum mass. "
            "Failed background/tidal attempts break lines. Missing values are not zero. "
            "Read the saved maximum-mass and convergence statuses before ML use.\n\n"
            "The dataset profile has one stellar stage and no per-case stellar refinement envelope; "
            "it is not STRICT certification. No solvers were run by plotting.\n",
            encoding="utf-8",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eos-data", type=Path, required=True)
