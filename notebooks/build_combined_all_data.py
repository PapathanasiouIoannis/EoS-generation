...
def build_combined_snapshot(
    ...
    *,
    ...
    current = Path(current_experiment).resolve(strict=True)
    target = Path(destination).resolve(strict=False)
    if precision not in {"quick", "strict"}:
        raise ValueError("precision must be 'quick' or 'strict'")
    try:
        current.relative_to(runs_root)
        target.relative_to(runs_root)
    except ValueError as exc:
        raise ValueError("current experiment and destination must remain below runs/") from exc
    if target.exists():
        raise FileExistsError(f"combined snapshot destination already exists: {target}")
    if target == runs_root or target == current or current in target.parents:
