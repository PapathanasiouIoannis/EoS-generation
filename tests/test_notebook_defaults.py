...
class NotebookDefaultsTests(unittest.TestCase):
    def test_notebooks_select_their_governed_40_point_profiles_and_remain_passive(self):
        for name, expected_precision in (
            ("bsk24_dataset.ipynb", "dataset_40_curves"),
            ("bsk24_experiment.ipynb", "dataset_40"),
        ):
            with self.subTest(notebook=name):
                ...
                self.assertEqual(expected_precision, values["PRECISION"])
                ...
                if name == "bsk24_experiment.ipynb":
                    geometry_count = (
                        len(values["CENTER"])
                        * len(values["WIDTH"])
                        * len(values["RAMP_WIDTH"])
                    )
                    self.assertEqual(125, geometry_count)
                    self.assertEqual(9, len(values["AMPLITUDES"]))
                ...
                if name == "bsk24_dataset.ipynb":
                    profile = _precision_profile(
                        values["PRECISION"], values["CALCULATION"]
                    )
                    self.assertTrue(profile["curve_only_output"])
                    code = "\n".join(
                        "".join(item["source"])
                        for item in document["cells"]
                        if item["cell_type"] == "code"
                    )
                    self.assertIn("create_student_view=False", code)
                    self.assertNotIn("eos_catalogue.py", code)
