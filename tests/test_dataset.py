...
class DatasetProfileTests(unittest.TestCase):
    def test_curve_only_40_requests_only_the_five_curve_families(self):
        full = _precision_profile("dataset_40", "stellar")
        curves = _precision_profile("dataset_40_curves", "stellar")
        self.assertIs(True, curves["curve_only_output"])
        self.assertEqual((full["thermodynamic_stages"][-1],), curves["thermodynamic_stages"])
        self.assertEqual(full["tov_stages"], curves["tov_stages"])
        self.assertEqual(
            {key: value for key, value in full.items() if key != "thermodynamic_stages"},
            {key: value for key, value in curves.items() if key not in {"thermodynamic_stages", "curve_only_output"}},
        )
        settings = ExperimentSettings.from_values(
            amplitudes=[0.0, 0.12], epsilon_match=80, center=450,
            width=500, ramp_width=350, calculation="stellar",
            precision="dataset_40_curves", diagnostics="off",
        )
        plan = plan_experiment(
            settings, output_root=ROOT / "runs/test-dataset-40-curves-no-write"
        )
        config = plan.child_plans[0].config
        self.assertTrue(config.curve_only_output)
        self.assertFalse(config.fixed_mass_background_requested)
        self.assertFalse(config.maximum_mass_requested)
        self.assertFalse(config.retained_stellar_profiles_requested)
        self.assertEqual(0, plan.estimates["fixed_mass_root_targets"])
        self.assertEqual(0, plan.estimates["fixed_mass_tidal_targets"])
        self.assertEqual(0, plan.estimates["maximum_mass_local_screen_targets"])
        self.assertEqual(80, plan.estimates["sampled_sequence_tidal_targets"])
        self.assertIn("Fixed-mass roots: not requested", plan.summary_text())
        self.assertIn("Maximum-mass refinement: not requested", plan.summary_text())
        with self.assertRaises(ValueError):
            ExperimentSettings.from_values(
                matter_model="cfl", epsilon_match="surface",
...
class DatasetPlotTests(unittest.TestCase):
    def test_direct_curve_plot_mode_writes_only_five_figures_and_provenance(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            experiment = fixture(root, "direct-curves", precision="dataset_40")
            ...
            with patch.object(plots.catalogue, "validate_source"), patch(
                ...
                result = plots.build_dataset_plots(
                    root, experiment, destination
                )
            self.assertEqual(5, result["generated_plot_count"])
            ...
            self.assertFalse((experiment.parent / "EOS_DATA").exists())
