...
def test_cfl_packet_adapters_retain_physical_identity_and_a0_arrays(
    ...
    inversion_calls = 0
    original_inversion = type(
        baseline
    ).quark_chemical_potential_from_energy_density

    def counted_inversion(self, values):
        nonlocal inversion_calls
        inversion_calls += 1
        return original_inversion(self, values)
...
def test_cfl_residual_and_convergence_evidence_are_model_specific(
    monkeypatch,
) -> None:
    child = _owner_child()
    deformation = next(
    ...
    from eos_generation._internal import cfl_thermodynamics

    monkeypatch.setattr(
        cfl_thermodynamics.np,
        "gradient",
        lambda *args, **kwargs: pytest.fail(
            "saved CFL residual arrays were recomputed"
        ),
    )
