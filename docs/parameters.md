...
| `precision` | Selects a governed numerical profile. | `"quick"`, `"strict"`, or experimental `"dataset"` / `"dataset_10_tighter"` / `"dataset_20"` / `"dataset_40"` / `"dataset_40_curves"` / `"dataset_relaxed"` / `"dataset_relaxed_80"`. |
...
| `diagnostics` | Requests governed endpoint extended stellar diagnostics. | BSk24: `"off"` or `"on"`, with `"on"` requiring `calculation = "stellar"`. CFL: exactly `"off"` in 1.2. |
...
The BSk24-only `dataset_40_curves` profile retains that final thermodynamic
grid, 40-point stellar grid, ODE tolerances, all-node tides, raw gate, and
reconstruction admissibility, while not requesting residual reporting,
fixed-mass roots, maximum-mass refinement, or retained radial profiles. It is
the focused BSk24 dataset-notebook default and is not available for CFL.
