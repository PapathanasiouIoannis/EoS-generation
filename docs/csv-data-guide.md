...
For short names in the standard and BSk24 dataset routes, use the sibling
`EOS_DATA/` generated by new notebook runs. The focused CFL dataset route has
only `CFL_DATASET/cfl_eos_data.csv` and `cfl_stellar_data.csv`; those tables
use `cfl_0`, `cfl_1`, ... labels and retain `experiment`, `geometry_id`, and
canonical `case_id` columns without a separate alias file or registry.
