...
unchanged public experiment facade. The unreleased
[`cfl_dataset.ipynb`](../notebooks/cfl_dataset.ipynb) extension is explicitly
experimental: its `dataset_40` path and shared H/C presentation adapter are
not covered by the acceptance evidence below until the deferred regression,
passivity, matched STRICT comparison, and clean-environment checks are run.
...
These results predate the unreleased CFL dataset extension described above
and do not qualify that new path. All then-existing CFL-focused tests pass,
including two passive run-all notebook executions
