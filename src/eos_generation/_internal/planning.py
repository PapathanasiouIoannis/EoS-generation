...
class BSk24TrialConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.curve_only_output and not self.stellar_enabled:
            raise ValueError("curve_only_output requires background TOV screening")
        if self.curve_only_output and self.extended_stellar_diagnostics_enabled:
            raise ValueError("curve_only_output is incompatible with extended stellar diagnostics")
    ...
    def fixed_mass_background_requested(self) -> bool:
        return self.stellar_enabled and not self.curve_only_output
    ...
    def thermodynamic_residual_reporting_requested(self) -> bool:
        return not self.curve_only_output
    ...
    def reference_compatible(self) -> bool:
        return (
            ...
            and not self.curve_only_output
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if not self.curve_only_output:
            # Preserve every established default configuration hash.  The
            # new execution mode is serialized only when explicitly active.
            data.pop("curve_only_output", None)
    ...
    if config.stellar_enabled:
        ...
        estimates["maximum_mass_local_screen_targets"] = int(
            ...
            * config.maximum_mass_initial_points
            if config.maximum_mass_requested
            else 0
        )
