...
def _cfl_thermodynamic_profile_frame(
    ...
    for case_id, eos in generated.items():
        ...
        direct_pressure = np.asarray(
            baseline.pressure_from_energy_density(epsilon), dtype=float
        )
        direct_density = np.asarray(
            baseline.baryon_density_from_energy_density(epsilon), dtype=float
        )
        direct_mu_b = np.asarray(
            baseline.baryon_chemical_potential_from_energy_density(epsilon),
            dtype=float,
        )
        frames.append(
            _profile_rows(
        ...
        for metric in _RESIDUAL_METRICS:
            ...
            for stage in stages:
                ...
                maxima[stage] = (
                    None
                    if eos is None
                    else _interior_maximum(_residual_arrays(eos)[metric])
                )
