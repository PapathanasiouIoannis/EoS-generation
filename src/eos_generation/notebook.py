...
class NotebookRun:
    ...
    def summary_text(self) -> str:
        ...
        if self.settings.precision == "dataset_40_curves":
            lines.append(
                "Curve-only dataset_40 profile: final 40-model stellar and "
                "thermodynamic grids; no fixed-mass, maximum-mass, residual, "
                "or retained radial-profile work."
            )
...
class NotebookSession:
    ...
    def execute(
        ...
        *,
        ...
        if not isinstance(create_student_view, bool):
            raise TypeError("create_student_view must be boolean")
        ...
        previous_progress = os.environ.get("EOS_GENERATION_PROGRESS")
        if show_progress:
            os.environ["EOS_GENERATION_PROGRESS"] = "1"
        try:
            result = runner(run.plan, execute=True)
        finally:
            if show_progress:
                if previous_progress is None:
                    os.environ.pop("EOS_GENERATION_PROGRESS", None)
                else:
                    os.environ["EOS_GENERATION_PROGRESS"] = previous_progress
        ...
        if status not in ("pass", "valid", "complete"):
            raise RuntimeError(f"completed experiment failed validation: {status}")
        if self._uses_production_workflow and create_student_view:
