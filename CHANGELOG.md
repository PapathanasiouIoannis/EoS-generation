...
- Both BSk24 notebooks now select `dataset_40` (40 sequence points, rtol=1e-10,
  atol=1e-12) and remain passive until a fresh preview is explicitly executed.
  The shared production case-worker cap and preview budget increase to six,
  still bounded by CPU/case count with nested pools disabled. Existing profile
  definitions, including multi-stage STRICT, scientific equations, acceptance
  gates and validation are unchanged. Historical notebook outputs are retained.
